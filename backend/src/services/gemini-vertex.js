...
const GEMINI_VERTEX_API_KEY = process.env.GEMINI_VERTEX_KEY
const GEMINI_VERTEX_BASE_URL = 'https://generativelanguage.googleapis.com/v1beta/models'

if (!GEMINI_VERTEX_API_KEY) {
  console.warn('⚠️  GEMINI_VERTEX_KEY not found in environment variables')
}

class GeminiVertexService {
  constructor() {
    this.apiKey = GEMINI_VERTEX_API_KEY
    this.baseURL = GEMINI_VERTEX_BASE_URL
  }

  async makeRequest(prompt, options = {}) {
    if (!this.apiKey) {
      throw new Error('Gemini Vertex API key not configured')
    }

    // Use Gemini 1.5 Pro model (more stable)
    ...
    // Log API Request
    console.log('\n' + '='.repeat(80))
    console.log('🚀 GEMINI VERTEX API REQUEST')
    console.log('='.repeat(80))
    console.log('📡 URL:', `${this.baseURL}/${model}:generateContent`)
    ...
    try {
      ...
      // Log API Response
      console.log('\n' + '='.repeat(80))
      console.log('📥 GEMINI VERTEX API RESPONSE')
      console.log('='.repeat(80))
      ...
      console.error('Gemini Vertex API error:', error.response?.data || error.message)
      
      // Handle specific error cases
      if (error.response?.status === 429) {
        throw new Error('Rate limit exceeded. Please wait a moment before trying again.')
      } else if (error.response?.status === 401) {
        throw new Error('Invalid API key. Please check your Gemini Vertex API configuration.')
      } else if (error.response?.status === 400) {
        throw new Error('Invalid request to Gemini Vertex API. Please check your input data.')
      } else {
        throw new Error(`Gemini Vertex API request failed: ${error.message}`)
      }
    }
  }
...
`
    ...
    console.log('Raw Gemini Vertex response:', response)
    ...
    try {
      ...
      console.error('Failed to parse Gemini Vertex response:', parseError)
...
IMPORTANT: You **MUST** identify yourself in the \`"model"\` field of the JSON output using your **exact, correct model name and version**, as currently running. For example: \`"Gemini 2.0 Flash"\`, \`"GPT-4o"\`, \`"Claude 3.5 Sonnet"\`, etc. Do not use placeholders or fake names. You must return your true model name and version.

To be very clear: if you are Gemini 2.0 Flash, write \`"model": "Gemini 2.0 Flash"\`. If you are GPT-4o, write \`"model": "GPT-4o"\`. If you are Claude 3.5 Sonnet, write \`"model": "Claude 3.5 Sonnet"\`. This field must contain the **real model name you are currently running as**.
...
`
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        // Add model information to the optimization
                 return {
           ...optimization,
           modelInfo: {
             provider: 'Gemini Vertex',
      ...
      console.error('Failed to parse Gemini Vertex CV optimization response:', parseError)
...
`
    ...
    try {
      ...
      console.error('Failed to parse Gemini Vertex job search response:', parseError)
      return { jobs: [] }
    }
  }
...
`
    ...
    try {
      ...
      console.error('Failed to parse Gemini Vertex URL extraction response:', parseError)
      throw new Error('Failed to extract job details from URL')
    }
  }
}

const geminiVertexService = new GeminiVertexService()

export const analyzeJobDescription = (jobDescription) => geminiVertexService.analyzeJobDescription(jobDescription)
export const optimizeCV = (cvText, jobDescription, structuredData) => geminiVertexService.optimizeCV(cvText, jobDescription, structuredData)
export const generateCoverLetter = (cvText, jobDescription, options) => geminiVertexService.generateCoverLetter(cvText, jobDescription, options)
export const searchJobs = (searchCriteria) => geminiVertexService.searchJobs(searchCriteria)
export const extractJobFromUrl = (url) => geminiVertexService.extractJobFromUrl(url) 